...
public class AuditLogAspect {
        ...
        try {
            ...
            new Thread(() -> {
                try {
                    ...
                    // Also create SystemActivityLog entry
                    createSystemActivityLog(className, methodName, ipAddress, userAgent, success, errorMessage);
                } catch (Exception ex) {
    ...
    private void createSystemActivityLog(String className, String methodName, String ipAddress, 
        ...
        try {
            ...
            String action = String.format("API: %s.%s", className, methodName);
            SystemActivityLog.LogLevel logLevel = success ? SystemActivityLog.LogLevel.INFO : SystemActivityLog.LogLevel.ERROR;
            String details = errorMessage != null ? "Error: " + errorMessage : "API call executed successfully";
            
            systemActivityLogService.logActivity(
                userId, username, action, "API_CALL", null, 
                ipAddress, userAgent, logLevel, details, success
            );
        } catch (Exception ex) {
            log.error("Failed to create system activity log: {}", ex.getMessage());
        }
    }
}