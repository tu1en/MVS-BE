...
public class ChatServiceImpl implements ChatService {
    ...
    public ChatMessage sendFileMessage(Long liveStreamId, Long senderId, String fileUrl, 
        ...
        try {
            ...
            ChatMessage message = ChatMessage.builder()
                    ...
                    .content("ÄÃ£ chia sáº» file: " + fileName)
                    .fileUrl(fileUrl)
                    .fileName(fileName)
                    .fileSize(fileSize)
                    .messageStatus(ChatMessage.MessageStatus.SENT)
    ...
    public ChatMessage handleRaiseHandResponse(Long messageId, Long teacherId, String response) {
        try {
            ...
            String notificationContent = "approve".equalsIgnoreCase(response) ?
                    raiseHandMessage.getSender().getFullName() + " Ä‘Æ°á»£c phÃ©p phÃ¡t biá»ƒu" :
                    raiseHandMessage.getSender().getFullName() + " khÃ´ng Ä‘Æ°á»£c phÃ©p phÃ¡t biá»ƒu lÃºc nÃ y";
        ...
        try {
            ...
            if (updatedRows > 0) {
                ...
                if (messageOpt.isPresent()) {
                    ...
                    String action = pinned ? "Ä‘Ã£ ghim" : "Ä‘Ã£ bá» ghim";
                    sendSystemNotification(message.getLiveStream().getId(), 
                            teacher.getFullName() + " " + action + " má»™t tin nháº¯n");
                }
                return true;
            }
            return false;
        ...
        try {
            ...
            if (updatedRows > 0) {
                ...
                if (messageOpt.isPresent()) {
                    ChatMessage message = messageOpt.get();
                    sendSystemNotification(message.getLiveStream().getId(), 
                            "Má»™t tin nháº¯n Ä‘Ã£ bá»‹ kiá»ƒm duyá»‡t: " + reason);
                }
                return true;
            }
            return false;
    ...
    public ChatMessage sendUserJoinNotification(Long liveStreamId, Long userId) {
        try {
            User user = getUserById(userId);
            String content = user.getFullName() + " Ä‘Ã£ tham gia phiÃªn há»c";
    ...
    public ChatMessage sendUserLeaveNotification(Long liveStreamId, Long userId) {
        try {
            User user = getUserById(userId);
            String content = user.getFullName() + " Ä‘Ã£ rá»i khá»i phiÃªn há»c";
            return sendSystemNotification(liveStreamId, content);
        } catch (Exception e) {
    ...
    public ChatMessage toggleChatEnabled(Long liveStreamId, Long teacherId, boolean enabled) {
        try {
            ...
            String content = enabled ? "Chat Ä‘Ã£ Ä‘Æ°á»£c báº­t" : "Chat Ä‘Ã£ Ä‘Æ°á»£c táº¯t";
            return sendSystemNotification(liveStreamId, content);
        } catch (Exception e) {
    ...
    public int clearAllMessages(Long liveStreamId, Long teacherId) {
        try {
            ...
            if (clearedCount > 0) {
                sendSystemNotification(liveStreamId, "Táº¥t cáº£ tin nháº¯n Ä‘Ã£ Ä‘Æ°á»£c xÃ³a bá»Ÿi giÃ¡o viÃªn");
            }
