...
public class GradeValidationServiceImpl implements GradeValidationService {
    ...
    public void validateScoreWithinLimits(Integer score, Assignment assignment) {
        ...
        log.debug("âœ… Score validation passed: {} <= {} for assignment {}", 
    ...
    public int fixInvalidScores() {
        log.info("ðŸ”§ Starting to fix invalid scores where score > assignment.points");
        ...
        log.info("ðŸ” Found {} submissions with scores to check", submissionsWithScores.size());
        ...
        for (Submission submission : submissionsWithScores) {
            ...
            if (score > maxPoints) {
                log.warn("ðŸš¨ Invalid score found: Submission {} has score {} > max points {} for assignment '{}'", 
                ...
                log.info("ðŸ”§ Fixed submission {}: score {} â†’ {} (capped to max points)", 
                    submission.getId(), score, maxPoints);
            }
        }
        
        log.info("âœ… Fixed {} invalid scores", fixedCount);
        return fixedCount;
    }

    @Override
    public int auditInvalidScores() {
        log.info("ðŸ” Auditing submissions for invalid scores");
        ...
        for (Submission submission : submissionsWithScores) {
            ...
            if (score > maxPoints) {
                log.warn("âŒ Invalid score: Submission {} has score {} > max points {} for assignment '{}'", 
                    submission.getId(), score, maxPoints, submission.getAssignment().getTitle());
                invalidCount++;
            }
        }
        
        log.info("ðŸ“Š Audit complete: {} invalid scores found out of {} submissions", 
