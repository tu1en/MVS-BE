...
public class ExplanationEvidenceServiceImpl implements ExplanationEvidenceService {
    ...
    public List<ExplanationEvidenceDto> uploadEvidenceFiles(Long explanationId, CreateExplanationDto createDto) {
        ...
        ViolationExplanation explanation = explanationRepository.findById(explanationId)
            .orElseThrow(() -> new IllegalArgumentException("KhÃ´ng tÃ¬m tháº¥y giáº£i trÃ¬nh vá»›i ID: " + explanationId));
        ...
        if (!createDto.isWithinSizeLimit()) {
            throw new IllegalArgumentException("Tá»•ng dung lÆ°á»£ng file vÆ°á»£t quÃ¡ " + maxTotalSizeMB + "MB");
        }
    ...
    public ExplanationEvidenceDto uploadSingleEvidence(Long explanationId, MultipartFile file, 
        ...
        if (!validateFile(file)) {
            ...
            throw new IllegalArgumentException("File khÃ´ng há»£p lá»‡: " + String.join(", ", errors));
        }
    ...
    public ExplanationEvidenceDto getEvidenceById(Long id) {
        ExplanationEvidence evidence = evidenceRepository.findById(id)
            .orElseThrow(() -> new IllegalArgumentException("KhÃ´ng tÃ¬m tháº¥y file báº±ng chá»©ng vá»›i ID: " + id));
    ...
    public void deleteEvidence(Long id, Long deletedBy) {
        ...
        if (!canAccessEvidence(id, deletedBy)) {
            throw new IllegalArgumentException("Báº¡n khÃ´ng cÃ³ quyá»n xÃ³a file báº±ng chá»©ng nÃ y");
        }
    ...
    public List<String> getFileValidationErrors(MultipartFile file) {
        ...
        if (file == null || file.isEmpty()) {
            errors.add("File khÃ´ng Ä‘Æ°á»£c Ä‘á»ƒ trá»‘ng");
            return errors;
        }
        ...
        if (file.getSize() > maxSizeBytes) {
            errors.add("File vÆ°á»£t quÃ¡ " + maxFileSizeMB + "MB");
        }

        // Check file type
        String contentType = file.getContentType();
        if (!isAllowedContentType(contentType)) {
            errors.add("Loáº¡i file khÃ´ng Ä‘Æ°á»£c há»— trá»£: " + contentType);
        }

        // Check filename
        String filename = file.getOriginalFilename();
        if (filename == null || filename.trim().isEmpty()) {
            errors.add("TÃªn file khÃ´ng há»£p lá»‡");
        } else if (filename.length() > 255) {
            errors.add("TÃªn file quÃ¡ dÃ i (tá»‘i Ä‘a 255 kÃ½ tá»±)");
        }
    ...
    public String generateDownloadUrl(Long id, Long userId) {
        ...
        if (!canAccessEvidence(id, userId)) {
            throw new IllegalArgumentException("Báº¡n khÃ´ng cÃ³ quyá»n truy cáº­p file nÃ y");
        }
