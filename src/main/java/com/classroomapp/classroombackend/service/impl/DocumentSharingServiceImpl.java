...
/**
 * Implementation cá»§a Document Sharing Service
 * Xá»­ lÃ½ upload, download, delete vÃ  realtime sync
 */
@Slf4j
@Service
...
public class DocumentSharingServiceImpl implements DocumentSharingService {
    ...
    public Resource downloadDocument(Long documentId, String requestedBy) {
        ...
        try {
            ...
            if (resource.exists()) {
                ...
                throw new RuntimeException("File khÃ´ng tá»“n táº¡i: " + document.getOriginalFileName());
            }
        } catch (MalformedURLException e) {
            log.error("Error creating file resource: {}", e.getMessage(), e);
            throw new RuntimeException("KhÃ´ng thá»ƒ táº¡o file resource: " + e.getMessage());
        }
    }
    ...
    public SharedDocument getDocumentById(Long documentId) {
        return sharedDocumentRepository.findById(documentId)
            .orElseThrow(() -> new IllegalArgumentException("Document khÃ´ng tá»“n táº¡i: " + documentId));
    }
    ...
    public void updatePresentationNavigation(Long documentId, int currentPage, String action, String controlledBy) {
        ...
        if (!document.canBePresented()) {
            throw new IllegalArgumentException("Document nÃ y khÃ´ng thá»ƒ present");
        }
    ...
    public void validateFile(MultipartFile file) {
        ...
        if (file.getSize() > MAX_FILE_SIZE) {
            throw new IllegalArgumentException("File size khÃ´ng Ä‘Æ°á»£c vÆ°á»£t quÃ¡ 10MB");
        }
        
        // Check MIME type
        String mimeType = file.getContentType();
        if (mimeType == null || !ALLOWED_MIME_TYPES.contains(mimeType)) {
            throw new IllegalArgumentException("File type khÃ´ng Ä‘Æ°á»£c há»— trá»£: " + mimeType);
        }
        
        // Check filename
        String filename = file.getOriginalFilename();
        if (filename == null || filename.contains("..")) {
            throw new IllegalArgumentException("Filename khÃ´ng há»£p lá»‡");
        }
