...
public interface RecruitmentApplicationRepository extends JpaRepository<RecruitmentApplication, Long> {
    @Modifying
    @Query("DELETE FROM RecruitmentApplication ra WHERE ra.jobPositionId = :jobPositionId")
    void deleteByJobPositionId(@Param("jobPositionId") Long jobPositionId);
    
    @Modifying
    @Query("DELETE FROM RecruitmentApplication ra WHERE ra.jobPositionId IN (SELECT jp.id FROM JobPosition jp WHERE jp.recruitmentPlan.id = :recruitmentPlanId)")
