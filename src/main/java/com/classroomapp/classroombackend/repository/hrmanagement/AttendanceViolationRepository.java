...
public interface AttendanceViolationRepository extends JpaRepository<AttendanceViolation, Long> {
    ...
    /**
     * Count violations in date range (for scheduler)
     * @param startDate start date
     * @param endDate end date
     * @return count of violations
     */
    long countByViolationDateBetween(LocalDate startDate, LocalDate endDate);
    
    /**
     * Count violations in date range - alias for scheduler compatibility
     * @param startDate start date
     * @param endDate end date
     * @return count of violations
     */
    default long countViolationsInDateRange(LocalDate startDate, LocalDate endDate) {
        return countByViolationDateBetween(startDate, endDate);
    }
    
    /**
     * Find old resolved violations for archival
     * @param cutoffDate cutoff date
     * @return list of old resolved violations
     */
    @Query("SELECT av FROM AttendanceViolation av " +
           "WHERE av.status = 'RESOLVED' " +
           "AND av.resolvedAt < :cutoffDate " +
           "ORDER BY av.resolvedAt ASC")
    List<AttendanceViolation> findOldResolvedViolations(@Param("cutoffDate") LocalDate cutoffDate);
}
