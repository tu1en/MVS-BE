...
/**
 * Debug controller để kiểm tra grading data consistency
 */
@RestController
...
public class DebugGradingController {
    ...
    /**
     * Debug endpoint để kiểm tra submissions với score nhưng isGraded null
     */
    @GetMapping("/submissions/inconsistent")
    public ResponseEntity<?> getInconsistentSubmissions() {
        log.info("🔍 DEBUG: Checking for submissions with score but isGraded inconsistency");
        ...
        log.info("🔍 DEBUG: Found {} submissions with scores", submissionsWithScores.size());
        ...
        log.info("🔍 DEBUG: Converted {} submissions to DTOs", submissionDtos.size());
        
        // Log details for debugging
        submissionDtos.forEach(dto -> {
            log.info("🔍 DEBUG: Submission ID={}, Score={}, IsGraded={}, AssignmentId={}, StudentName={}", 
    ...
    /**
     * Debug endpoint để kiểm tra submissions cho một classroom cụ thể
     */
    @GetMapping("/classroom/{classroomId}/submissions")
    public ResponseEntity<?> getClassroomSubmissions(@PathVariable Long classroomId) {
        log.info("🔍 DEBUG: Getting submissions for classroom {}", classroomId);
        ...
        log.info("🔍 DEBUG: Found {} submissions for classroom {}", submissions.size(), classroomId);
    ...
    /**
     * Debug endpoint để kiểm tra một submission cụ thể
     */
    @GetMapping("/submission/{submissionId}")
    public ResponseEntity<?> getSubmissionDetails(@PathVariable Long submissionId) {
        log.info("🔍 DEBUG: Getting details for submission {}", submissionId);
        ...
        try {
            ...
            log.info("🔍 DEBUG: Submission {} - Raw hasScore: {}, DTO isGraded: {}", 
                    submissionId, submission.getScore() != null, dto.getIsGraded());
            
            return ResponseEntity.ok(debugInfo);
            
        } catch (Exception e) {
            log.error("🔍 DEBUG: Error getting submission {}: {}", submissionId, e.getMessage());
            return ResponseEntity.badRequest().body("Error: " + e.getMessage());
        }
    }

    /**
     * Debug endpoint để fix existing data inconsistency
     */
    @GetMapping("/fix-data")
    public ResponseEntity<?> fixDataInconsistency() {
        log.info("🔧 DEBUG: Starting data consistency fix");
        
        List<Submission> submissionsWithScores = submissionRepository.findAll().stream()
                .filter(s -> s.getScore() != null)
                .collect(Collectors.toList());
        
        log.info("🔧 DEBUG: Found {} submissions with scores to verify", submissionsWithScores.size());
        ...
        for (Submission submission : submissionsWithScores) {
            ...
            if (submission.getScore() != null && submission.getGradedAt() == null) {
                ...
                fixedCount++;
                log.info("🔧 DEBUG: Fixed submission {} - set gradedAt", submission.getId());
            }
        }

        log.info("🔧 DEBUG: Fixed {} submissions", fixedCount);
    ...
    /**
     * Debug endpoint để audit invalid scores (score > assignment.points)
     */
    @GetMapping("/audit-invalid-scores")
    public ResponseEntity<?> auditInvalidScores() {
        log.info("🔍 DEBUG: Starting audit for invalid scores");
    ...
    /**
     * Debug endpoint để fix invalid scores (cap them to assignment.points)
     */
    @GetMapping("/fix-invalid-scores")
    public ResponseEntity<?> fixInvalidScores() {
        log.info("🔧 DEBUG: Starting fix for invalid scores");
    ...
    /**
     * Debug endpoint để lấy submission với assignment details
     */
    ...
    public ResponseEntity<?> getSubmissionWithAssignment(@PathVariable Long submissionId) {
        log.info("🔍 DEBUG: Getting submission {} with assignment details", submissionId);
        ...
        try {
            ...
            log.error("🔍 DEBUG: Error getting submission with assignment {}: {}", submissionId, e.getMessage());
            return ResponseEntity.badRequest().body("Error: " + e.getMessage());
        }
    }
}
